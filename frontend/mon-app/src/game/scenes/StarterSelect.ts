...
export class StarterSelect extends Scene {
  ...
  create() {
    ...
    this.titleText = this.add
      .text(width / 2, 60, "CHOISIS 3 POKÉMON (0/3)", {
        ...textStyle,
        fontSize: "24px",
      })
      .setOrigin(0.5);
    ...
    for (let i = 0; i < 3; i++) {
      this.add
        .circle(startX + i * 150, 160, 45, 0x334155)
    ...
    this.availableMonsters.forEach((key, index) => {
      ...
      cardBg.on("pointerover", () => {
        if (
          this.selectedTeam.length < this.maxSelections ||
          this.selectedTeam.includes(key)
        ) {
    ...
    this.confirmButton = this.add
      .text(width / 2, height - 80, "▶ VALIDER L'ÉQUIPE ◀", {
        ...textStyle,
        fontSize: "20px",
        color: "#64748b",
      })
