...
export class Battle extends Scene {
  ...
  init(data: any) {
    ...
    const firstAliveIndex = this.playerTeam.findIndex(
      (pokemon: any) => pokemon.currentHp > 0,
    );
  ...
  create() {
    ...
    kb.on("keydown-ESC", () => {
      if (this.escAction) this.escAction();
    });
  ...
  private makeEnemyArea() {
    ...
    this.enemyHpBar = this.add
      .rectangle(infoX, infoY + 40, 200, 12, 0x44dd44)
      .setOrigin(0, 0.5);
  ...
  private makePlayerArea() {
    ...
    this.playerHpBar = this.add
      .rectangle(infoX, infoY + 40, currentBarWidth, 12, 0x44dd44)
      .setOrigin(0, 0.5);
  ...
  private makeTextBox() {
    this.add
      .rectangle(512, 680, 1004, 120, 0x000000, 0.7)
  ...
  private showAttackMenu() {
    ...
    sp.moveIds.forEach((moveId, i) => {
      this.makeMenuButton(280 + i * 200, 660, getMove(moveId).name, () =>
  ...
  private onEnemyFainted() {
    const next = this.enemyTeam.findIndex(
      (p, i) => i > this.enemyActive && p.currentHp > 0,
    );
    if (next !== -1) {
  ...
  private showForcedSwitchMenu(aliveIndices: number[]) {
    ...
    aliveIndices.forEach((idx, i) => {
      ...
      this.makeMenuButton(520 + i * 220, 660, label, () =>
        this.forcedSwitch(idx),
      );
    });
    this.escAction = null;
  }
  ...
  private makeMenuButton(
    ...
    const bg = this.add
      .rectangle(x, y, 180, 40, disabled ? 0x555555 : 0x0066cc)
      .setStrokeStyle(2, 0xffffff);
    const txt = this.add
      .text(x, y, label, {
        fontFamily: "Arial Black",
    ...
    if (!disabled) {
      ...
      bg.on("pointerover", () => {
        this.navIndex = this.navButtons.findIndex((n) => n.bg === bg);
        this.refreshHighlight();
      });
